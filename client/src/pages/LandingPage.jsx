...
const features = [
  ...
  { icon: Bot, title: 'AI Advisor', desc: 'Get personalized repayment advice and debt-trap warnings powered by Gemini or DeepSeek.' },
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <nav className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated ? (
            <Link to="/dashboard" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
              Go to Dashboard
            </Link>
          ) : (
            <>
              <Link to="/login" className="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white font-medium">Sign in</Link>
              <Link to="/register" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium">Get started</Link>
            </>
          )}
        </div>
      </nav>
