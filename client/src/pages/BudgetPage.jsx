...
import { PlusCircle, Trash2, PiggyBank, TrendingUp, TrendingDown } from 'lucide-react';
    ...
    api.get(`/budget?month=${month}&year=${year}`)
      .then((res) => {
        if (res.data) {
          ...
          setIncome('');
          setExpenses([{ category: 'Housing', amount: '' }]);
        }
      })
      .catch(() => {})
      ...
      {fetching ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 space-y-5">
          <div>
            ...
            <input type="number" min="0" value={income} onChange={(e) => setIncome(e.target.value)} placeholder="5000" className={`${inputCls} w-full`} />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pt-2 border-t border-gray-100 dark:border-gray-700">
            <div className="text-center">
              ...
              <p className="font-bold text-gray-900 dark:text-white">${(parseFloat(income) || 0).toLocaleString()}</p>
            </div>
            <div className="text-center">
              <TrendingDown className="w-4 h-4 text-red-500 mx-auto mb-1" />
              <p className="text-xs text-gray-500 dark:text-gray-400">Expenses</p>
              <p className="font-bold text-gray-900 dark:text-white">${totalExpenses.toLocaleString()}</p>
            </div>
            <div className="text-center">
          ...
          {surplus > 0 && (
            <p className="text-xs text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg px-3 py-2">
              You have <strong>${surplus.toLocaleString()}</strong> available for extra debt payments this month. Use the Simulator to see how it accelerates your payoff.
            </p>
          )}
